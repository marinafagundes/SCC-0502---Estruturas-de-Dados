...
int alterar(t_chave c, t_chave novo_valor, t_abb *t) {
    if ((*t) == NULL) {
            printf("Chave não encontrada. Não é possível alterar.\n");
            return NAO_EXISTE;
    }
    ...
    if (c == (*t)->chave) {
            t_apontador p = *t;
            (*t)->chave = novo_valor;
            free(p); // Libera o nó que foi substituído
            printf("Chave atualizada com sucesso.\n");
            return SUCESSO;
    } else if (c < (*t)->chave) {
            return alterar(c, novo_valor, &(*t)->esq);
    } else {
            return alterar(c, novo_valor, &(*t)->dir);
    }
}
