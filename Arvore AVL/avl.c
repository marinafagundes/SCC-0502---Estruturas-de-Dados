...
static int update_altura(t_avl *t) {
    return max(retorna_h(&(*t)->esq),
                        retorna_h(&(*t)->dir)) + 1;
}

static void rot_dir(t_avl *t) {
...
void limpar(t_avl *t) { // limpar memoria (free em tudo)
...
static int cria_raiz(t_chave c, t_avl *t) {

    (*t) = (t_avl) malloc(sizeof(t_no));
    if (*t == NULL) {
...
static void achaMaiorEsqTroca(t_abb *t, t_avl *sub_t) {
